...
class Player(object):
    ...
    def send_message(self, player_str):
        for position in self.positions:
            msg = player_str + ' ' + str(self.position[0]) + ' ' + str(self.position[1])
...
def consoles():
    ...
    while True:
        ...
        if command == 'client':
            client_direction = message.split()[1]
...
def GameMain():
    ...
    while True:
        ...
        # 메세지 받기
        global client_direction
        if client_direction == 'left':
            player2.change_direction(LEFT)
        elif client_direction == 'right':
            player2.change_direction(RIGHT)
        elif client_direction == 'up':
            player2.change_direction(UP)
        elif client_direction == 'down':
        ...
        # msg = "length " + str(player1.length) +  + str(player2.length)
        # client.sendall(msg.encode())
